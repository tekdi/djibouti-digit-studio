...
const InboxSearchComposer = ({ configs, additionalConfig, onFormValueChange = () => {}, showTab, tabData, onTabChange, customizers = {} }) => {
  ...
  if (!hasRun.current) {
    ...
    if (
      !Object.keys({ ...Digit?.Customizations?.commonUiConfig?.[configs.apiDetails.moduleName] }).length > 0 &&
      Object.keys(customizers).length > 0
    ) {
  ...
  return (
    <InboxContext.Provider value={{ state, dispatch }}>
      <div className="digit-inbox-search-composer-header-action-wrapper">
        ...
        {Digit.Utils.didEmployeeHasAtleastOneRole(configs?.actions?.actionRoles) && (
          <Button
            ...
            icon="Add"
            onClick={() => {
              history.push(`/${window?.contextPath}/${userType}/${configs?.actions?.actionLink}`);
            }}
      ...
      {configs?.footerProps?.showFooter && Digit.Utils.didEmployeeHasAtleastOneRole(configs?.footerProps?.allowedRolesForFooter) && (
        <Footer
          actionFields={configs?.footerProps?.actionFields
            ?.filter((btnConfig) => Digit.Utils.didEmployeeHasAtleastOneRole(btnConfig?.allowedRoles))
            ?.map((btnConfig, index) => (
              <Button
                key={index}
                icon={btnConfig?.icon}
                ...
                {...btnConfig}
                onClick={(event) => configModule?.footerActionHandler?.(index, event)}
              />
            ))}
          className={configs?.footerProps?.className || ""}
          maxActionFieldsAllowed={configs?.footerProps?.maxActionFieldsAllowed}
          setactionFieldsToLeft={configs?.footerProps?.setactionFieldsToLeft}
          setactionFieldsToRight={configs?.footerProps?.setactionFieldsToRight}
          sortActionFields={configs?.footerProps?.sortActionFields ? configs?.footerProps?.sortActionFields : true}
