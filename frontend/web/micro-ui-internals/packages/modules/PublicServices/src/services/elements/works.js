  ...
  viewEstimateScreen: async (t, tenantId, estimateNumber) => {
    ...
    const sample = {
      ...
      values: [
        ...
        { title: "EVENTS_MAP_LABEL", map: true, value: "N/A" },
    ...
    const tableRowsNonSor = nonSOR?.map((row, index) => {
      return [index + 1, row?.description, t(`ES_COMMON_UOM_${row?.uom}`), row?.unitRate, row?.noOfunit, row?.amountDetail[0]?.amount?.toFixed(2)];
    });
    ...
    const nonSorItems = {
      ...
      tableStyles: {
        ...
        cellStyle: [
          {},
          { width: "40vw", whiteSpace: "break-spaces", wordBreak: "break-all" },
          {},
          { textAlign: "right" },
          { textAlign: "left" },
          { textAlign: "right" },
