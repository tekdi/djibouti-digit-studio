...
import APEInstructionSheetModal from "./APEInstructionSheetModal";
import { useAPEInstructionSheetData } from "./hooks/useAPEInstructionSheetData";

const APEInstructionSheetCard = ({ service, state, t, isViewOnly = false }) => {
  ...
  } = useAPEInstructionSheetData(applicationNumber, serviceCode);
          ...
          <div className="flex flex-col gap-6 p-6 min-h-full">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              {instructionData.finalOpinion && (
                <div className="rounded-xl border border-gray-100 bg-gray-50/50 p-4 md:col-span-2">
                  <span className="text-xs font-medium uppercase tracking-wide text-gray-500">
                    Approbation
                  </span>
                  <p className="mt-2">
                    <span
                      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${
                        instructionData.finalOpinion === "APPROVED"
                          ? "bg-emerald-100 text-emerald-700"
                          : "bg-red-100 text-red-700"
                      }`}
                    >
                      {instructionData.finalOpinion === "APPROVED"
                        ? "Plans d'exécution approuvés"
                        : "Plans d'exécution non approuvés"}
                    </span>
                  </p>
                </div>
              )}
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-gray-500">
                      ({instructionData.history.length}{" "}
                      {instructionData.history.length === 1 ? "modification" : "modifications"})
                    </span>
                    {isHistoryOpen ? (
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="rounded-xl border border-primary/10 bg-primary/5 p-4">
              ...
              <p className="mt-2 text-sm font-semibold text-primary">
                12 documents (dont 5 sous-points)
              </p>
            </div>
          </div>
...
APEInstructionSheetCard.propTypes = {
  service: PropTypes.string.isRequired,
  state: PropTypes.string.isRequired,
  t: PropTypes.func.isRequired,
};

export default APEInstructionSheetCard;
