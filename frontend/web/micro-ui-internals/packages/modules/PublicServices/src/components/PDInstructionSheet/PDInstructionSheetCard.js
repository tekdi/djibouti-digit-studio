...
import PDInstructionSheetModal from "./PDInstructionSheetModal";
import { usePDInstructionSheetAPI } from "./hooks/usePDInstructionSheetAPI";

const PDInstructionSheetCard = ({ service, state, t, isViewOnly = false }) => {
  const tenantId = Digit.ULBService.getCurrentTenantId();
  const { serviceCode, applicationNumber } = Digit.Hooks.useQueryParams();
  const { getFiche } = usePDInstructionSheetAPI(tenantId, serviceCode, applicationNumber);
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-3">
                <h3 className="text-xl font-semibold text-gray-900">Fiche d'instruction — Détails du Permis de Démolition</h3>
                {isSubmitted ? (
...
PDInstructionSheetCard.propTypes = {
...
export default PDInstructionSheetCard;
