...
import { useInstructionSheetAPI } from "./useInstructionSheetAPI";

/**
 * Hook pour gérer les données de la Fiche d'instruction (SRA)
 */
export const useInstructionSheetData = (applicationNumber, serviceCode) => {
  const [instructionData, setInstructionData] = useState(null);
  const [isSubmitted, setIsSubmitted] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  const tenantId = Digit?.ULBService?.getCurrentTenantId();
  const { getInstructionSheet } = useInstructionSheetAPI(tenantId, serviceCode, applicationNumber);
