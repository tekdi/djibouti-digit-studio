...
const PFInstructionSheetModal = ({
  ...
  const { isLoading, submitFiche } = usePFInstructionSheetAPI(tenantId, serviceCode, applicationNumber);
  ...
  const validate = () => {
    ...
    if (!formData.closureType || !String(formData.closureType).trim()) e.closureType = "Champ requis";
    ...
    <div className="fixed inset-0 bg-gray-50 z-[1000] flex flex-col" onClick={onClose}>
      ...
      <div className="bg-white w-full h-full overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 lg:px-8 py-4 border-b border-gray-200 bg-white">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Fiche d'instruction — Détails du Permis de Clôture</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="md:col-span-2">
              {field("pcoNumber", "Numéro du Permis de Construire", {
                placeholder: "Ex: P8-PCL-N°18/2026",
                hint: "P8-PCL-N°18/2026",
              })}
            </div>
            {field("applicantName", "Nom et Prénoms du Pétitionnaire", { required: true, placeholder: "Nom complet" })}
            {field("closureType", "Type de clôture", { required: true, placeholder: "Ex: Mur en parpaings, grillage…" })}
            ...
            {field("perimeter", "Périmètre de la parcelle (mètre linéaire)", { placeholder: "Ex: 80" })}
            <div>
...
PFInstructionSheetModal.propTypes = {
...
export default PFInstructionSheetModal;
