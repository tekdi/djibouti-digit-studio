...
const CitizenApplicationsPendingPayment = () => {
  ...
  // Filter applications to show only pending payment ones
  const filteredApplications = useMemo(() => {
    return applications
      .filter(app => {
        const applicationStatus = getAppStatus(app);
        const isPending = applicationStatus === "AWAITING_CITIZEN_PAYMENT";
        ...
        return isPending && matchesSearch && matchesBusinessService && matchesDate;
      })
  ...
  const businessServices = useMemo(() => {
    const pending = applications.filter(a => getAppStatus(a) === "AWAITING_CITIZEN_PAYMENT");
    return [...new Set(pending.map(app => app.businessService))].filter(Boolean);
  }, [applications]);
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-5">
        ...
        <div className="mb-6">
          <p className="text-gray-600">
            {filteredApplications.length} demande{filteredApplications.length !== 1 ? 's' : ''} en attente de paiement
          </p>
        </div>
