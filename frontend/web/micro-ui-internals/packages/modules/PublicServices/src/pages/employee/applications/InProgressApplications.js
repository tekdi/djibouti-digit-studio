...
import InProgressStatisticsCards from "./components/InProgressStatisticsCards";
...
const InProgressApplications = () => {
  ...
  // Filter applications to show only in-progress applications
  const inProgressApplications = useMemo(() => {
    return applications.filter((app) => {
      const status = app.ProcessInstance?.state?.applicationStatus;
      // Show applications that are not completed and not unassigned
  ...
  // Get unique business services for filter (only from in-progress applications)
  const availableBusinessServices = useMemo(() => {
    const services = [...new Set(inProgressApplications.map((app) => app.businessObject?.businessService))];
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-6">
      <InProgressHeader isRefreshing={isRefreshing} onRefresh={refreshApplications} />

      <InProgressStatisticsCards applications={inProgressApplications} totalCount={totalCount} />
